...
import recipe.uft.torch_functional as verl_F
from verl.utils.model import compute_position_id_with_mask

logger = logging.getLogger(__name__)


class UFTDataset(Dataset):
    """
    Implements a dataset loader for Unified Fine-Tuning (UFT), a novel post-training paradigm
    that unifies Supervised Fine-Tuning (SFT) and Reinforcement Fine-Tuning (RFT).
    ...
    def __init__(
        ...
        is_train: bool = True,
        total_training_steps_hint: Optional[int] = None,
        lower_prob: float = 0.05,
        upper_prob: float = 0.95,
    ):
        ...
        self.total_training_steps_hint = total_training_steps_hint
    ...
    def __getitem__(self, item):
        ...
        hint_slice_prop = 0.0
        sliced_hint = ""
        if "hint" in row_dict and self.is_train:
            hint = row_dict.pop("hint")
            num = min(5, len(hint))
            separation = [round(_ / num * len(hint)) for _ in range(num + 1)]
            separation[-1] = len(hint)

            if self.timestep > self.total_training_steps_hint:
                slice_length = 0
            else:
                current_timestep = self.timestep
                prob = self.lower_prob + 0.5 * (self.upper_prob - self.lower_prob) \
                            * (1 + np.cos(np.pi * current_timestep / self.total_training_steps_hint))
